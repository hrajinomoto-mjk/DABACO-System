...
const LOCAL_PRIMARY_LOGO_URL = '/ajinomoto-global-brand.png';
const LOCAL_SVG_LOGO_URL = '/ajinomoto-eat-well-live-well.svg';
...
export const AjinomotoLogo: React.FC<AjinomotoLogoProps> = ({
  ...
  const [currentSrc, setCurrentSrc] = useState<string>(LOCAL_PRIMARY_LOGO_URL);
  const [useVectorFallback, setUseVectorFallback] = useState(false);

  const handleImageError = () => {
    // Cascade fallback: Local PNG -> Local SVG -> Remote Wikimedia -> Inline Pure Vector
    if (currentSrc === LOCAL_PRIMARY_LOGO_URL) {
      setCurrentSrc(LOCAL_SVG_LOGO_URL);
    } else if (currentSrc === LOCAL_SVG_LOGO_URL) {
      setCurrentSrc(AJINOMOTO_OFFICIAL_LOGO_URL);
    } else {
  ...
  const renderVectorLogo = () => {
    // Wordmark & Emblem Vector (Authentic Ajinomoto Typography)
    if (variant === 'symbol' || variant === 'horizontal' || variant === 'full') {
      return (
        <svg
          viewBox="0 0 77.2 11.2"
          className="h-full w-auto aspect-[77/11] select-none"
  ...
  return (
    <div className={`relative inline-flex items-center justify-center ${className}`}>
      {/* Renders the official Ajinomoto Group Global Brand logo */}
      {!useVectorFallback ? (
        <img
          src={currentSrc}
          alt={alt}
          ...
          style={{ filter: outlineFilter }}
          className="h-full w-auto max-h-full max-w-full object-contain select-none transition-transform"
        />
      ) : (
        renderVectorLogo()
      )}
